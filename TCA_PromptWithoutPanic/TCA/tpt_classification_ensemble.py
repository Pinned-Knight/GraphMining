...
def test_time_tuning(model, inputs, optimizer, scaler, args):
    ...
    for j in range(args.tta_steps):
        if 'tpt' in args.run_type:
            with torch.cuda.amp.autocast():
                if args.cocoop:
                    ...
                    #     model1 = torch.nn.DataParallel(model, device_ids = [0, 1, 2, 3, 4])
                    output = model(inputs) 
        ...
        if 'ctpt' in args.run_type:
            ...
            # loss += 0.5 * torch.linalg.norm(model.l2_norm_attribute_mean_training)
            # loss += 30 * model.l2_norm_attribute_mean_training.mean() # Intradispersion loss (GPU - 0)
            # loss += 20 * model.l2_norm_attribute_mean_training.mean() # (GPU - 1)
            # loss += model.l2_norm_attribute_mean_training.mean()  #Imagenet Variants
        
        if 'tca' in args.run_type:
            if output == None and output2 == None:
                single_output = model(args.image)

            alpha = args.alpha
            beta = args.beta
            loss += (-alpha* model.l2_norm_mean_training)
            # loss += 0.5 * torch.linalg.norm(model.l2_norm_attribute_mean_training)
            loss += beta * model.l2_norm_attribute_mean_training.mean() # Intradispersion loss (GPU - 0)

            
           
           
           # (Nine) Imagenet ensemble 3 attributes Resnet GPU=5,6
     
           # (Fourth) Imagenet ensemble 3 attributes VITB-16 GPU=2,3
           
          
           # bash scripts/test_tpt_ctpt_ds.sh R
           
           
           
            # (Eight) Imagenet Sketch ensemble 2 attributes VITB-16 GPU=0,1

            #(Ten) Imagenet ensemble 2 attributes VITB-16 GPU=4,7
...
def main_worker(gpu, args, result_dict):
    ...
    "Pets": "Attributes/OxfordPets_2_V.json",
    "Caltech101": "Attributes/Caltech101_2_V.json",
    "UCF101": "Attributes/UCF101_2_V.json",
    "Cars": "Attributes/StanfordCars_2_V.json",
    "eurosat":"Attributes/EuroSAT_2_V.json", 
    "Aircraft": "Attributes/FGVCAircraft_2_V.json",
    "I" : "Attributes/ImageNet_2_V.json", 
    "V" : "Attributes/ImageNet_2_V.json", 
    "A" : "Attributes/ImageNet_2_V.json",
    "R" : "Attributes/ImageNet_2_V.json", 
    "K": "Attributes/ImageNet_2_V.json", 
    }
    atrribute_file_path = attributes_path_dict[args.test_sets]
    with open(atrribute_file_path, 'r') as file:
        json_data = file.read()
    ...
    if args.cocoop:
        ...
        list_of_models = []
        list_of_models.append(get_coop(args.arch, args.test_sets, args.gpu, args.n_ctx, args.ctx_init, args.num_attributes, args.multi_gpu, attributes))
        for i in ["a photo of the", "a picture of a", "a picture of the"]:
            list_of_models.append(get_coop(args.arch, args.test_sets, args.gpu, args.n_ctx, i, args.num_attributes, args.multi_gpu, attributes))
        ...
        # model_state = None
        list_of_model_states = [None, None, None, None]
    ...
    if args.cocoop:
        ...
        for model in list_of_models:
            trainable_param = model.prompt_learner.parameters()
            optimizer = torch.optim.AdamW(trainable_param, args.lr)
            list_of_optimizers.append(optimizer)
            optim_state = deepcopy(optimizer.state_dict())
            list_of_optim_state.append(optim_state)
    ...
    for set_id in datasets:
        ...
        results[set_id] = test_time_adapt_eval(val_loader, list_of_models, list_of_model_states, list_of_optimizers, list_of_optim_state, scaler, args, result_dict)
...
def test_time_adapt_eval(val_loader, list_of_models, list_of_model_states, list_of_optimizers, list_of_optim_state, scaler, args, result_dict):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if 'ctpt' or 'tca' in args.run_type:
            args.image = image
        ...
        if not args.cocoop: # no need to reset cocoop because it's fixed
            ...
            for (i, model) in enumerate(list_of_models):
                list_of_optimizers[i].load_state_dict(list_of_optim_state[i])

                test_time_tuning(model, images, list_of_optimizers[i], scaler, args)
        else:
            with torch.no_grad():
                with torch.cuda.amp.autocast():
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                if args.cocoop:
                    ...
                    list_of_outputs = []
                    for model in list_of_models:
                        list_of_outputs.append(model(image))

                    output = sum(list_of_outputs)/len(list_of_outputs)
        ...
        if 'ts' not in args.run_type:
            ...
            # print(softmax_output == output)
            
            # print("softmax_output", softmax_output)

        elif 'ts' in args.run_type:
            if 'tca' not in args.run_type:
                if 'ViT' in args.arch:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_type' , type=str, default='baseline_tpt', choices=['baseline', 'tpt', 'tpt_ctpt', 'tpt_ts', 'tpt_tca'])
    ...
    # ------------------------------------------------
     # added args for tca
    parser.add_argument('--alpha' , type=float, default=0.0, help='alpha for tca')
